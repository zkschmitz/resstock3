...
for year in ${years[@]}; do
    ...
    if [ -d "$OUTPUT_PROJECT_DIR" ] && [ "$(ls -A "$OUTPUT_PROJECT_DIR")" ]; then
        ...
        # run resstock
        cd $ROOT_DIR
        openstudio workflow/run_analysis.rb -y project_local_pjm/local.yml -k -o
        ...
        cd $ROOT_DIR/outputs
        find local_pjm -name '*.csv' -o -name "cli_output.log" | cpio -pdm $OUTPUT_PROJECT_DIR/inputs

        cd $OUTPUT_PROJECT_DIR/inputs
        mv local_pjm ResStock_outputs
